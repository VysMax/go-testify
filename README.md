...
Сервис возвращает список кафе по запросу. В запросе указано количество возвращаемых кафе и город, где находится заведение общественного питания. Обработчик принимает этот запрос и формирует ответ. Если какие-то параметры указаны некорректно (нет такого города, неправильно указано количество), обработчик вернёт ошибку. Сервер будет ожидать обращение по пути `/cafe`. В GET-параметрах ожидается:

- `count` — количество кафе, которые нужно вернуть
- `city` — город, в котором нужно найти кафе

Если в параметре `count` указано больше, чем есть всего, возвращаются все доступные кафе. В сервисе используется только один город - `moscow`, в котором находится всего 4 кафе. Реализованы три теста:
